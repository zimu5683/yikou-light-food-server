...
> ⚠️ **2026-09-18 修订**：第 2 条已从"绝对值"改回**累加 + 账本幂等**，原因见
> `WPS-CLOUD-SYNC-PLAN.md` 的「与初版计划的差异」第 1 条。以下为**当前**规则。

对每个本地子表 → 对应的云端表：

0. **先核对批次日期**：本地每行的「周一~周日」标记（= 这批要送的那天的周几）
   与目标日期不符时**整张表拒绝写入**（连通讯记号都不写），提示重跑「订单处理」；
1. **按【名字 + 电话】匹配客户**（两者都对上才算同一个人）；同一个人本地有几行，
   **云端就写几行**、各标当天 1（用户 2026-09-18 确认：一晚两单、一单两份都算
   "这天吃两餐"，这样「闪时送下单」逐行出单才会送两餐）；
2. **旧客户**（每一行按"槽位"处理：本地第 i 行 ↔ 云端这个人的第 i 行）：
   - **总餐次 = 云端现值 + 本次增量**；增量 = 本地这一行的「餐次」 −
     账本里本批这一槽位已同步的餐次（账本无记录 = 首次）。例：云端 5 + 本地新下 6 → **11**
   - 增量 0（同批重复上传）→ 一个格子都不写（幂等靠账本，不再靠"绝对值"）
   - 增量 < 0（退单）→ 不减，只警告
   - **目标日期格**：空 → 写 `1`；**协作者已写 `0`（当天不送）等值 → 一个字节都不改**；
3. **新客户**：插到第 3/4 行之间再整表重排，填：名字 / 地址 / 电话 / 类型 / 餐种 /
   总餐次 / 目标日期格=`1`
...
| 11 | **总餐次写绝对值 → 吃掉已付过钱的餐**（云端 5 + 新下 6 被写成 6） | 2026-09-18 改回**累加 + 账本幂等**；账本从"留痕"升级为幂等锚点（`local` 字段） |
| 12 | **把协作者写的 `0`（当天不送）当成"还没写"，改成 `1`** | 日期格里任何非 1 的非空值都是协作者的明确决定：**只读不写**；回读校验按"原值不变"核对（下游「闪时送下单」按 `日期格 == 1` 出单，多写一个 1 就多送一餐） |
| 13 | 累加语义下**拿错本地表会把同一批餐重复加一遍** | 本地表每行的「周一~周日」标记就是这批的日期；不符时整张表拒绝写入（`plan.blocked_reason`） |
...
### 13.6 上色规则修正（2026-09-15 晚，用户反馈）

**现象**：表格里「日期列」与「类型」之间有时夹着一列**空列**（正式表东湖中餐
当前的 L 列就是它：日期排到 K，类型在 M）。旧逻辑是"逐列照抄模板行的底色"，
而接口**不返回空单元格**、读不到那一列的颜色 → 新行的那一格保留了插入时从上一行
继承的颜色，看起来就是"有一格没涂到"。

**修法**（用户原话："直接涂 A 列到餐种所在列的全部格子的底色"）：

| 行 | 涂法 |
|---|---|
| 经济餐 | **第 A 列 ~「餐种」列整段**刷模板底色（取模板行第 A 列的颜色）；**总餐次/已出餐/剩余餐**三列固定金黄；其余（备注）照旧 |
| 豪华餐 | A~备注 整行金黄（不变） |

金黄带改为**按列的身份**（总餐次/已出餐/剩余餐）固定涂金，不再依赖模板行那三格
有没有内容 —— 实测有客户行的总餐次是空的，模板刚好选到它就会把黄带学成白色。

`build_format_ops(spec, ..., plain_to=, band_cols=)`；`apply_plan` 传
`plan.columns["kind"]` 与那三列的列号。

**离线仿真抓到的自身 bug**：`fills` 里是 `"#AARRGGBB"` 字符串，而
`spec["fill_default"]` 已经是整数 —— 起初又对它调了一次 `_argb_to_int`，
`str(4294967295)` 被当成十六进制 → 涂出 `#94967295` 这种鬼色。已修并加注释。

**验证**：新增用例 `test_paint_covers_gap_column_between_dates_and_type`（带空列的
表，断言空列被涂到、白底区间正好到「餐种」列）；云同步用例 80 个全过；
`tools/simulate_wps_fixture.py` 的验收标准同步改成新规则并通过。

**注意**：这次改动**尚未构建、未提交、未发布**（2026-09-15 21:5x 时工作区里还有
另一条线在做 3.4.0「闪时送云端名单导入」，`app/__init__.py` 已被那边改成 3.4.0）。
下次构建部署时这份修复会一起进去。
