"""独立反证用合成闪时送子进程（不联网/不真实下单）。

用法::

    python3 tests/independent_sss_batch_child.py <work_dir> <journal_path> <mode>

- 平台状态写入 ``<work_dir>/platform.json``（POST 计数 + 订单记录）；
- ``<work_dir>/hidden`` 存在时列表接口永远返回空，模拟最终一致性延迟；
- ``<work_dir>/signal`` 在 POST 落库并计数后被写下，供父进程做确定性同步；
- ``mode=crash`` 时 POST 落库并写 signal 后 ``os._exit(17)``，模拟首进程崩溃；
- 其余 mode 正常返回 ``{"success": true}``。

本文件只由同目录的独立反证测试/探针调用，不进入生产路径。
"""
...
def main() -> int:
    ...
    # 故障注入：仅由独立 mutation 测试开启，正常探针不设置这些环境变量。
    if os.environ.get("INDEP_MUTATE_ORIGIN_RAW") == "1":
        sss_uncertain._normalise_origin = (  # type: ignore[assignment]
            lambda value: str(value or "").strip())
    if os.environ.get("INDEP_MUTATE_MIRROR_ALL") == "1":
        _original_legacy_paths = sss_runner.legacy_uncertain_paths
        sss_runner.mirror_uncertain_paths = (  # type: ignore[assignment]
            lambda config, authoritative=None:
                _original_legacy_paths(config, authoritative))
    ...
    class _SyntheticClient:
        """只实现 runner 用到的客户端方法；POST 写入共享平台文件。"""
