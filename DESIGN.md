...
## Project adaptation: Yikou Light Food desktop app

This project is a Chinese-language desktop application built with Tkinter. Use the Starbucks-inspired tokens above as visual direction, adapted to desktop constraints:

- Keep the warm cream canvas and deep green brand accents, while preserving strong contrast for Windows and macOS system displays.
- Prefer Segoe UI (with Arial or Helvetica fallback) for the interface. Keep Chinese labels concise and use clear hierarchy rather than all-caps text.
- Use rounded cards, restrained shadows, and pill-shaped primary actions where Tkinter supports them. Do not introduce web-only responsive breakpoints or browser navigation patterns.
- Keep task configuration, actions, search, and run logs visually grouped. The run log should remain high-contrast and easy to scan during automation.
- Preserve existing behavior and user-facing Chinese copy when refining `app/gui.py`; this document guides visual changes and does not authorize changing automation logic.

## Attribution

This design guidance is adapted from [VoltAgent/awesome-design-md](https://github.com/VoltAgent/awesome-design-md), specifically `design-md/starbucks/DESIGN.md`. The upstream repository is MIT licensed; Copyright (c) 2026 VoltAgent.
